...
{\color{red}In this primarily worked with rigid cages, which might not be the best representation of the cages. As the cages pack together they are subject to forces that might change their geometry\cite{hasell2016porous}. To alleviate a lot of these concerns in our work, we have ran molecular dynamic (MD) simulations on four cages (CC2, CC3, CC4 and CC5) using force field parameters from a polymer specific consistent force field (PCFF)\cite{sun1998compass} and a cage specific force field (CSFF)\cite{holden2012bespoke} in the NVT ensemble. Further details can be found in Sec. \ref{sec:flexibility}. Snapshots of each cage were gathered while they were fluctuating during the MD simulation and mapped onto a lower-dimensional latent space. As can be seen in Figure \ref{fig:pca_space_with_flex}, each cage occupies a latent basin as it's allowed to fluctuate. The basin size is determined by the degree of flexibility of each cage. Even though the flexibility has an effect on the latent space we have generated, a static look at the cages does provide valuable insight while reducing computational expenses.
}
