...
To explore the effects of flexibility/ structural fluctuations of cages on their latent representations, we performed molecular dynamics (MD) simulations in the NVT ensemble ($T=$298 K) on four empty, isolated cages (\textbf{CC2}, \textbf{CC3}, \textbf{CC4} and \textbf{CC5}) using a polymer specific consistent force field (PCFF)\cite{sun1998compass}\todo{arni cite appropriate} and a cage specific force field (CSFF)\cite{holden2012bespoke}. See Sec.~\ref{sec:flexibility} for details. We gathered snapshots of each cage while they were fluctuating in the MD simulation, computed their 3D porosity images, then projected them onto a $\nu=2$-dimensional latent space. Fig.~\ref{fig:pca_space_with_flex} shows that flexible cages, on the basis of the MD snapshots, each occupy a \emph{region} in latent space. The size of the region is determined by the degree of flexibility of the cage; for example, the larger cage CC5 explores a larger region of latent space than \textbf{CC3}. Moreover, the latent regions may overlap (as the case with \textbf{CC3} and \textbf{CC4}), indicating that, a fraction of the time, the cages exhibit a very similar cavity shape. 
