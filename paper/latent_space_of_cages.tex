...
We computationally scanned porous cage molecules to generate 3D images of their nano-cavities. Then, we encoded them into low-dimensional, latent vector representations predictive of their properties.
\end{tocentry}
...
Leveraging the singular value decomposition, in an unsupervised manner, we then learn across all cages an approximate, lower-dimensional subspace in which the 3D porosity images congregate.
The ``eigencages'' are the set of orthogonal, characteristic 3D porosity images that span this lower-dimensional subspace, ordered in terms of importance. A latent representation/encoding of each cage follows by approximately expressing it as a combination of the eigencages. 
...
\centering
	\subfloat[][\textbf{B15} \cite{greenaway2018high}]{\includegraphics[width=0.3\columnwidth]{B15_raw.png}}
	\subfloat[][\textbf{B24} \cite{greenaway2018high}]{\includegraphics[width=0.3\columnwidth]{B24_raw.png}}
	\qquad
	\subfloat[][\textbf{C6} \cite{greenaway2018high}]{\includegraphics[width=0.3\columnwidth]{C6_raw.png}}
	\subfloat[][\textbf{CD2} \cite{crini2014history}]{\includegraphics[width=0.3\columnwidth]{CD2_raw.png}}
	\qquad
	\subfloat[][\textbf{HC1} \cite{horng2009preparation}]{\includegraphics[width=0.3\columnwidth]{HC1_raw.png}}
	\subfloat[][\textbf{WC1} \cite{ding2015targeted}]{\includegraphics[width=0.3\columnwidth]{WC1_raw.png}}	
	\caption{Example 3D porosity images. (a-f) The molecular structure of a cage (name and experimental reference in subcaption) is shown along with a contour (0.5) of the 3D porosity image (orange). Bounding box shows $[-20 \text{ } \angstrom, 20 \text{ } \angstrom]^3$ dimension of the snapshot, consistent for all cages.
...
Alignment is also required for images of human faces for eigenfaces, so that e.g. the nose and eyes appear in the same region for each face \cite{zhang2008eigenfaces}. First, we translate each cage molecule so that its center of mass is at the origin. Second, we determine how to rotate each cage about its center of mass so that cages with similar cavity shapes are aligned.
...
The singular value decomposition (SVD) \cite{muller2004singular,kalman1996singularly,strang1993introduction} enjoys use in genomics \cite{alter2000singular}, recommender systems \cite{koren2008factorization}, and image processing \cite{muller2004singular}. We provide a pedagogical overview of the SVD for the reader's convenience. We then reason that the SVD of our data matrix $\mathbf{A}$ identifies an approximate lower dimensional subspace in which the 3D porosity images lie (the latent space) and a latent representation of the porosity of each cage molecule.
...
\label{eqn:svd_r}
\end{equation} where now $\mathbf{\Sigma}_r$ is a diagonal $r\times r$ matrix that contains only the $r$ non-zero singular values of $\mathbf{A}$ down its diagonal in a non-increasing order, $\mathbf{U}_r=[\mathbf{u}_1\hspace{1mm} \mathbf{u}_2\hspace{1mm} \cdots \hspace{1mm}\mathbf{u}_r]$ is a $c\times r$ matrix, and $\mathbf{V}_r=[\mathbf{v}_1\hspace{1mm} \mathbf{v}_2\hspace{1mm}\cdots \hspace{1mm}\mathbf{v}_r]$ is a $g^3\times r$ matrix (retaining only the first $r$ singular vectors). Writing eqn.~\ref{eqn:svd_r} using an outer product expansion expresses $\mathbf{A}$ as a sum of rank-one matrices:
\begin{equation}
\mathbf{A} = \displaystyle \sum_{i=1}^r \sigma_i \mathbf{u}_i\mathbf{v}_i^T.
\label{eqn:sum_rank_1}
\end{equation} The singular values appear in eqn.~\ref{eqn:sum_rank_1} as weights on the rank-one matrices, formed by the outer product of corresponding left and right (unit) singular vectors, used to construct the matrix $\mathbf{A}$. This emphasizes that the singular vectors are ordered in terms of significance.

We compute the singular value decomposition using the \texttt{svd} function in Julia \cite{julia}.
...
We highly encourage readers to explore our interactive visualization of the latent space at \url{simonensemble.github.io/latent-cage-space}; upon hovering the mouse over a point in latent space, an image of the cage structure displays to facilitate interpreting the latent space. Fig.~\ref{fig:latent_space} highlights a few salient clusters. The remaining clusters are shown in Fig.~\ref{fig:latent_space2}. The clustering of the cages in latent space according to the shape of their cavities is consistent with our intuition. As this clustering is learned automatically, we can easily generalize to thousands of porous cages instead of manually grouping them together. 
...
Together, Figs.~\ref{fig:latent_space_S_Xe_Kr}, \ref{fig:cage_space_colored_by_diams_2D}, and \ref{fig:cage_space_colored_by_nb_windows} show that our latent representations of porous cage molecules are useful for predicting properties that are correlated with the shape of the cavity (as is the case for Xe/Kr selectivity \cite{sikora2012thermodynamic,simon2015best}) and thus lend a meaningful notion of similarity between two cage molecules.
...
\begin{figure}
\centering
	\subfloat[][\textbf{NC2}]{\includegraphics[width=0.165\columnwidth]{NC2_latent.png}}
	...
	\subfloat[][\textbf{DC1}]{\includegraphics[width=0.165\columnwidth]{DC1_latent.png}}
...
\subsection{Flexible cages occupy a region of latent space}
\label{sec:flexible_MD}
Thus far, we viewed the cage molecules as rigid. But, often cages are flexible to some degree \cite{chen2014separation,camp2016transition,holden2014gas,holden2016understanding}, and the shape of their cavity fluctuates \cite{chen2014separation}, which influences adsorption \cite{witman2017influence}.
To explore the effects of flexibility/ structural fluctuations of cages on their latent representations, we performed molecular dynamics (MD) simulations in the NVT ensemble ($T=298$ K) on four empty, isolated cages (\textbf{CC2}, \textbf{CC3}, \textbf{CC4} and \textbf{CC5}) using the polymer consistent force field (PCFF) \cite{sun1994ab,sun1994force,sun1995ab} as implemented by \texttt{DL\_FIELD v4.3} \cite{Yong2016dlfield} and the cage specific force field (CSFF)\cite{holden2012bespoke}. We gathered snapshots of each cage while they were fluctuating in the MD simulation, computed their 3D porosity images, then projected them onto a $\nu=2$-dimensional latent space. See Sec.~\ref{sec:flexibility} for details. Fig.~\ref{fig:pca_space_with_flex} illustrates that flexible cages, on the basis of the MD snapshots, each occupy a \emph{region} in latent space-- instead of a point, as in the static view of cages. The size of the region is determined by the degree of flexibility of the cage; for example, the larger cage \textbf{CC5} explores a larger region of latent space than \textbf{CC3}. Moreover, the latent regions may overlap (as the case with \textbf{CC3} and \textbf{CC4}), indicating that, a fraction of the time, the cages exhibit a very similar cavity shape. 
...
\centering
	\subfloat[][]{
		\includegraphics[width=0.8\columnwidth]{../MD_figures/cc5_progression.pdf}
	}\qquad
\subfloat[][]{
	\includegraphics[width=0.8\columnwidth]{../PCA_latent_cage_space_with_flexible_cages_2D_maintext.pdf}
	}
	\caption{Flexible cages explore an area in latent space. (a) Snapshots of the structure of an isolated, empty cage \textbf{CC5} undergoing thermal fluctuations during a molecular dynamics simulation. (b) The gray points are the $\nu=2$-dimensional latent representations of the (rigid) cages learned by SVD, rows of $\mathbf{U}_2\mathbf{\Sigma}_2$.
The 3D porosity images of 400 snapshots of cages \textbf{CC2}, \textbf{CC3}, \textbf{CC4} and \textbf{CC5} (colored) during molecular dynamics simulations are projected onto this two-dimensional latent space. To precisely define the axes, the 3D porosity image of cage $k$ in the scatter plot is best-approximated in terms of a linear combination of the first two eigencages as $\mathbf{c}_k \approx \mathbf{\bar{c}} + \ell_{1,k} \mathbf{v}_1 + \ell_{2,k} \mathbf{v}_2$. Each fluctuating cage explores a latent \emph{region} determined by the set of cavity shapes explored while undergoing thermal fluctuations. \textbf{MC6}, an outlier, is omitted to avoid stretched axes.
	} \label{fig:md_snapshots}
\end{figure}
...
C.M.S. and A.S. thank the School of Chemical, Biological, and Environmental Engineering at Oregon State University for start-up funds. M.T.H. thanks the Pete and Rosalie Johnson Internship Program at Oregon State University. A.H.P.Y. thanks the URSA Engage Program at Oregon State University. We thank the anonymous reviewers for helpful comments that led to improvements in our results and the text.
