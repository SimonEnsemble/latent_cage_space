...
\centering
	\subfloat[][B15 \cite{greenaway2018high}]{\includegraphics[width=0.3\columnwidth]{../data/grids/B15_raw.png}}
	\subfloat[][B24 \cite{greenaway2018high}]{\includegraphics[width=0.3\columnwidth]{../data/grids/B24_raw.png}}
	\qquad
	\subfloat[][C6 \cite{greenaway2018high}]{\includegraphics[width=0.3\columnwidth]{../data/grids/C6_raw.png}}
	\subfloat[][CD2 \cite{crini2014history}]{\includegraphics[width=0.3\columnwidth]{../data/grids/CD2_raw.png}}
	\qquad
	\subfloat[][HC1 \cite{horng2009preparation}]{\includegraphics[width=0.3\columnwidth]{../data/grids/HC1_raw.png}}
...
{\color{red} t-SNE is an effective dimensionality reduction algorithm designed to preserve the local structure of the high-dimensional data in the low-dimensional embedding, but it also often captures global structure \cite{maaten2008visualizing}. In contrast to principal component analysis (PCA) (i.e. choosing $\nu=2$ in eqn.~\ref{eq:Anu} for a 2D latent space), t-SNE is non-linear and thus can retain/capture complicated structure in the $\nu=23$-dimensional space. Moreover, in contrast to t-SNE, PCA prioritizes maintaining large distances in data space in the latent space rather than local structure. The only tunable parameter in t-SNE is the perplexity, which can be conceptualized as the number of neighbors each data point has \cite{maaten2008visualizing,wattenberg2016how}. We find that our clusters of cages are fairly robust to our choice of the perplexity (5, in the range recommended in Ref.~\cite{maaten2008visualizing}).
} Fig.~\ref{fig:latent_space} shows the resulting two-dimensional embedding of the 3D void space images of the porous cages; each cage appears as a point in this \emph{latent cage space}.
