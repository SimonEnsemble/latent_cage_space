...
% made with image zoom 1.4. see saved VisIT session in data/grids. mesh width 4, see snapshot.vtk
	...
	\\
	\subfloat[][$\mathbf{v}_1$]{\includegraphics[width=0.275\columnwidth]{../data/grids/eigencage_1.png}\label{fig:eigencage1}}
...
\subsection{\color{red}The flexibility of cages}
{\color{red}In this work we have primarily worked with rigid cages, which might not be the best representation of the cages. As the cages pack together they are subject to forces that might change their geometry and structure\cite{hasell2016porous}, which rigid cages are incapable of describing. To explore the effects of flexibility and fluctuation of the cages, molecular dynamic (MD) simulations were performed on four empty, isolated cages (CC2, CC3, CC4 and CC5) using force field parameters from a polymer specific consistent force field (PCFF)\cite{sun1998compass} and a cage specific force field (CSFF)\cite{holden2012bespoke} in the NVT ensemble. Further details can be found in Sec. \ref{sec:flexibility}. Snapshots of each cage were gathered while they were fluctuating during the MD simulation and mapped onto a lower-dimensional latent space. Figure \ref{fig:pca_space_with_flex} shows that each cage occupies a latent basin as it's allowed to fluctuate. The basin size is determined by the degree of flexibility of each cage. Even though a rigid cage is unable to describe any fluctuations, it does serve as an average configuration of the cage, which provides valuable insight while requiring significantly less computational power.
}
